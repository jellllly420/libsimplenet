...
TEST(runtime_uring_test, wait_readable_suspends_and_resumes) {
    simplenet::runtime::uring_event_loop loop;
    if (!loop.valid()) {
        GTEST_SKIP() << "io_uring unavailable for runtime loop";
    }
...
TEST(runtime_uring_test, async_accept_read_write_echo_round_trip) {
    simplenet::runtime::uring_event_loop loop;
    if (!loop.valid()) {
        GTEST_SKIP() << "io_uring unavailable for runtime loop";
    }
    ...
    for (std::size_t i = 0; i < outbound.size(); ++i) {
        outbound[i] = static_cast<std::byte>((i * 7U) % 251U);
    }
...
TEST(runtime_uring_test, read_with_timeout_returns_timed_out_error) {
    simplenet::runtime::uring_event_loop loop;
    if (!loop.valid()) {
        GTEST_SKIP() << "io_uring unavailable for runtime loop";
    }
